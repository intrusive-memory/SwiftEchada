...
> **Provenance:** moved from `SwiftVoxAlta/docs/CHARACTER-STUDIO.md` on 2026-04-30. VoxAlta scoped itself down to a thin VoiceProvider for SwiftHablare; voice creation/casting belongs in SwiftEchada (`echada cast`). This is the design as it last lived in VoxAlta — verbatim, no edits — kept here so the casting-side design work isn't lost. Treat it as a **proposal**, not a shipped feature: at time of move, none of the components below (`CharacterProfile`, `CharacterAnalyzer`, `VoiceDesigner`, audition UI, multi-candidate generation) existed in code, in either repo. Decide whether to build it here, spin it into its own library, or archive the design before reaching for any of these names.
