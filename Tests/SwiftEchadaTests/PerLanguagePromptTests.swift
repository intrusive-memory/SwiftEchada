...
struct PerLanguagePromptTests {
  ...
  @Test func memberWithLocalizedVoiceOnlyIsCastableForThatLanguage() {
    // Member has voices["es"] but no voicePrompt — castable for "es"
  ...
  @Test func memberWithNeitherVoiceNorDescriptionIsSkipped() {
    // Member has no voices and no voicePrompt — castable for nothing
    let member = CastMember(
      character: "NARRATOR",
      voicePrompt: nil,
      voices: [:]
  ...
  @Test func memberWithEmptyVoiceDescriptionAndNoLocalizedVoiceIsSkipped() {
    // Empty string voicePrompt counts as absent — still not castable
    let member = CastMember(
      character: "NARRATOR",
      voicePrompt: "",
      voices: [:]
  ...
  @Test func memberWithLocalizedVoiceNotCastableForOtherLanguageWithNoBase() {
    // Has voices["es"] but no voicePrompt — NOT castable for "en"
    let member = CastMember(
      character: "NARRATOR",
      voicePrompt: nil,
      voices: ["es": ["warm spanish narrator"]]
  ...
  // MARK: - Criterion 7: base voicePrompt only — falls back for all languages

  @Test(
    "A base voicePrompt makes a member castable for every requested language",
    arguments: [
      ["es", "en"],
      ["es", "en", "fr"],
    ])
  func memberWithBaseDescriptionIsCastableForAllRequestedLanguages(_ requested: [String]) {
    ...
    let member = CastMember(
      character: "NARRATOR",
      voicePrompt: "Deep, warm baritone with measured pacing",
      voices: [:]
    )
  ...
  @Test func memberWithBothLocalizedAndBaseIsCastableForAll() {
    // Has voices["es"] AND voicePrompt — castable for all requested languages
    let member = CastMember(
      character: "NARRATOR",
      voicePrompt: "Warm storyteller",
  ...
  @Test func promptSelectionUsesLocalizedVoiceWhenAvailable() {
    // Member has voices["es"] AND a base voicePrompt.
    ...
    // "es" → localized value
    let promptForEs = member.voice(for: "es") ?? member.voicePrompt
    #expect(promptForEs == localizedEs)

    // "fr" → falls back to base
    let promptForFr = member.voice(for: "fr") ?? member.voicePrompt
    #expect(promptForFr == base)
  }

  @Test func promptSelectionFallsBackToBaseWhenLocalizedAbsent() {
    // Member has only a base voicePrompt — all languages fall back to it.
    ...
    let promptForEn = member.voice(for: "en") ?? member.voicePrompt
  ...
  @Test func promptSelectionEnglishDefaultPathUnchangedWithNoLocalizedPromptNoAccent() {
    ...
    // voice(for:"en") is nil → falls back to voicePrompt → composeVoicePrompt(base, nil) == base.
    // The resulting voice string is byte-for-byte identical to voicePrompt.
    ...
    let selected = member.voice(for: "en") ?? member.voicePrompt
  ...
  @Test func accentComposesOntoLocalizedPromptNotBase() {
    ...
    let selectedEs = member.voice(for: "es") ?? member.voicePrompt
  ...
  @Test func accentComposesOntoBaseWhenNoLocalizedPromptExists() {
    ...
    let selectedFr = member.voice(for: "fr") ?? member.voicePrompt
  ...
  @Test func noAccentOnLocalizedPromptReturnsLocalizedVerbatim() {
    ...
    let selected = member.voice(for: "es") ?? member.voicePrompt
    let composed = composeVoicePrompt(base: selected!, accent: nil)
  ...
  @Test func localizedOnlyMemberIsCastableForRegionalTagViaBaseSubtag() {
    // Localized-only member (no voicePrompt) with voices["es"] must NOT skip
  ...
  @Test func regionalTagPrefersBaseSubtagPromptOverGenericDescription() {
    // With BOTH a base voicePrompt and a localized voices["es"], a regional
    ...
    let selected = localizedVoicePrompt(for: member, language: "es-MX") ?? member.voicePrompt
  ...
  @Test func regionalTagWithNoBaseSubtagPromptFallsBackToDescription() {
    ...
    let selected = localizedVoicePrompt(for: member, language: "fr-CA") ?? member.voicePrompt
