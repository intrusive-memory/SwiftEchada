...
struct VoxFileIntegrationTests {
  ...
  @Test func createSingleModelVoxFile() throws {
    ...
    // Build
    let vox = makeVoxFile(
      name: "NARRATOR", description: "Deep authoritative male narrator voice with gravitas",
      gender: "male")
    ...
    try vox.add(
      cloneData, at: VoxPaths.clonePrompt(slug: slug),
      metadata: [
    ...
    try vox.add(
      sampleData, at: VoxPaths.sampleAudio(slug: slug),
      metadata: [
  ...
  @Test func createMultiModelVoxFile() throws {
    ...
    let vox = makeVoxFile(
      name: "HERO", description: "Young confident voice with warmth and energy", gender: "female")

    // Add 0.6b model
    for (slug, model) in [("0.6b", model06B), ("1.7b", model17B)] {
      try vox.add(
        syntheticClonePrompt(), at: VoxPaths.clonePrompt(slug: slug),
        metadata: [
          "key": VoxPaths.embeddingKey(slug: slug, suffix: "clone-prompt"),
          "model": model,
          "engine": "qwen3-tts",
      ...
      try vox.add(
        syntheticWAV(), at: VoxPaths.sampleAudio(slug: slug),
        metadata: [
          "key": VoxPaths.embeddingKey(slug: slug, suffix: "sample-audio"),
          "model": model,
          "engine": "qwen3-tts",
    ...
    // Both models present
    #expect(
      embeddings[VoxPaths.embeddingKey(slug: "0.6b", suffix: "clone-prompt")]?.model == model06B)
    #expect(
      embeddings[VoxPaths.embeddingKey(slug: "1.7b", suffix: "clone-prompt")]?.model == model17B)
  ...
  @Test func appendModelToExistingVoxFile() throws {
    ...
    let vox = makeVoxFile(
      name: "VILLAIN", description: "Menacing low voice with subtle threat underneath",
      gender: "male")

    let clone06 = syntheticClonePrompt()
    try vox.add(
      clone06, at: VoxPaths.clonePrompt(slug: "0.6b"),
      metadata: [
    ...
    try vox.add(
      syntheticWAV(), at: VoxPaths.sampleAudio(slug: "0.6b"),
      metadata: [
    ...
    try reopened.add(
      clone17, at: VoxPaths.clonePrompt(slug: "1.7b"),
      metadata: [
    ...
    try reopened.add(
      syntheticWAV(), at: VoxPaths.sampleAudio(slug: "1.7b"),
      metadata: [
