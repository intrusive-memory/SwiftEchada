...
/// `echada prompt` — examine the screenplay source material and write a
/// `voicePrompt` for each cast member in PROJECT.md.
///
/// This is the inverse of `cast`: where `cast` consumes each member's
...
/// Foundation Model to write the voice-design brief that `cast` will later use.
///
...
public struct PromptCommand: AsyncParsableCommand {
